...
DonkeyCarイメージを作成するためのインストール手順を解説しています。<br>
...
{{ download_link('https://faboplatform.github.io/DonkeyDocs/7.SDカードイメージ作成/ssh', 'ssh') }}をダウンロードし、SDカードにコピーします。`ssh`ファイルの中身はただの空ファイルですが、`ssh`という名前のファイルがSDカードに存在すると、sshでのアクセスが可能になる設定になります。
...
{{ download_link('https://faboplatform.github.io/DonkeyDocs/7.SDカードイメージ作成/wpa_supplicant.conf', 'wpa_supplicant.conf') }}をダウンロードします。ファイルを編集してSSIDとパスワードを接続先のWiFiルータの値に書き直します。
